...
function init(){
    // toDraw			whether the animation should run or not
    // pointNum			number of radial anchor points
    // radius			the base radius for the anchor points
    ...
    var canvas = document.getElementById('canvas'), toDraw, pjs, pointNum, radius, anchors, stageCenter, webCenter, rays, avgRadius, spacingNoise, barPointAmt, strands, strandNoise, physics, stats, statsInterval;
	...
	document.body.appendChild( stats.getDisplayElement() ); // container is a DOM Element
    ...
    ******************************* */
	...
	this.pointSpacing = 40;							// amount of space between strands (35 is good). This may have to depend on other things later such as the average line length or something.
	...
	pjs.ellipse(this.edgePoint.x, this.edgePoint.y, 10, 10);
	this.edgePoint.lock();
 ...
 ************************************************* */
	...
	this.strandColor = pjs.color(Math.random()*100 + 105);
	this.strokeW = undefined;
    
	n = pjs.noise(strandNoise);
	this.strandAlpha = pjs.map(n, 0, 1, 100, 250);
